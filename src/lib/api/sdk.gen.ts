...
export const autoScrapeItem = <ThrowOnError extends boolean = false>(options: Options<AutoScrapeItemData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<AutoScrapeItemResponses, AutoScrapeItemErrors, ThrowOnError>({
        requestValidator: async (data) => {
            return await zAutoScrapeItemData.parseAsync(data);
        },
        responseValidator: async (data) => {
            return await zAutoScrapeItemResponse.parseAsync(data);
        },
        security: [
            {
                name: 'x-api-key',
