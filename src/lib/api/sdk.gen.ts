 ...
 */
export const updateNewReleasesItems = <ThrowOnError extends boolean = false>(options?: Options<UpdateNewReleasesItemsData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<UpdateNewReleasesItemsResponses, UpdateNewReleasesItemsErrors, ThrowOnError>({
        requestValidator: async (data) => {
            return await zUpdateNewReleasesItemsData.parseAsync(data);
        },
        responseValidator: async (data) => {
            return await zUpdateNewReleasesItemsResponse.parseAsync(data);
        },
        security: [
            {
                name: 'x-api-key',
...
/**
 * Reindex item with Composite Indexer to pick up new season & episode releases.
